...
int main(int argc, char* argv[]) {
  ...
  if (!obj.randomize_with(obj.car() == car_type_enum::MERCEDES, obj.color() == BLUE)) {
    LOG(WARNING) << "A Mercedes cannot be blue!";
  }
  if (!obj.randomize_with(obj.car() == car_type_enum::BMW, obj.power() <= 60)) {
    LOG(ERROR) << "There is no such weak BMW!";
  }
  if (!obj.randomize_with(obj.price() < 20)) {
    LOG(FATAL) << "German cars cannot be that cheap!";
  }
  return 0;
}
